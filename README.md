...
|Model|Pros|Cons|
|:-----|:----|:----|
|Logistic Regression|本身有明确的概率公式，由于会对连续变量进行分箱处理，所以会降低过拟合的程度，增强模型泛化能力|对于多分类、或者非线性的决策边界时，表现效果有限。|
|Naive Bayes|假定所有输入属性的重要性是相等的，且彼此是独立的，这个前提满足的情况下，贝叶斯算法往往十分精确|实际情况是，经常发生不满足条件独立性的情况，并且缺少可用的概率数据。|
|Decision Tree|好理解，不需要对样本数据做任何假设，相对更适合处理非数值型数据|结果有时会很复杂，同一个变量在树生长的时候反复用了好几次导致不好解释，尤其当样本数据是数值型时，数据的微小变化都会影响每个节点变量以及阈值的选择，结果较不稳定|
|Knn|基于类比的学习，可以对缺失值进行处理|但维度过高时，也容易过拟合，本身不建立X与Y的量化公式，算法费时较长|
|SVM|使用一种非线性映射，所以对于非线性决策边界的识别能力是比较强的，本身不容易过拟合|算法本身效果依赖核心点的选择，所以扩展到大数据集时相对比较费时|
...
* [各种机器学习算法的应用场景分别是什么（比如朴素贝叶斯、决策树、K 近邻、SVM、逻辑回归最大熵模型）？ 京东白条](https://www.zhihu.com/question/26726794/answer/544818558?utm_source=wechat_session&utm_medium=social&utm_oi=650812049907650560)
